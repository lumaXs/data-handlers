...
type FieldOutput<F extends string | FieldConfig> = F extends string
   ? string
...
type RequiredKeys<TShape extends SchemaShape> = {
   [K in keyof TShape]: TShape[K] extends { optional: true }
      ? TShape[K] extends { default: unknown }
         ? K
         : never
      : K
}[keyof TShape]
...
export class Schema<TShape extends SchemaShape> {
    ...
    */
   extend<TExtra extends SchemaShape>(
      extra: TExtra,
   ): Schema<Omit<TShape, keyof TExtra> & TExtra> {
      return new Schema({ ...this.#shape, ...extra } as Omit<
         TShape,
         keyof TExtra
      > &
         TExtra)
 ...
 */
export function schema<TShape extends SchemaShape>(
   shape: TShape,
): Schema<TShape> {
