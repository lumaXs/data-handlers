...
// ─── CPF ─────────────────────────────────────────────────────────────────────
describe('plugin: cpf', () => {
  it('formata CPF com dígitos', () =>
    expect(normalize({ type: 'cpf', value: '11144477735' })).toBe(
      '111.444.777-35',
    ))
  it('formata CPF já formatado', () =>
    expect(normalize({ type: 'cpf', value: '111.444.777-35' })).toBe(
      '111.444.777-35',
    ))
  it('lança para dígitos repetidos', () =>
    expect(() => normalize({ type: 'cpf', value: '00000000000' })).toThrow(
      TypeError,
    ))
  it('lança para dígito verificador errado', () =>
    expect(() => normalize({ type: 'cpf', value: '11144477700' })).toThrow(
      TypeError,
    ))
...
describe('plugin: cnpj', () => {
  it('formata CNPJ com dígitos', () =>
    expect(normalize({ type: 'cnpj', value: '11222333000181' })).toBe(
      '11.222.333/0001-81',
    ))
  it('formata CNPJ já formatado', () =>
    expect(normalize({ type: 'cnpj', value: '11.222.333/0001-81' })).toBe(
      '11.222.333/0001-81',
    ))
  it('lança para dígitos repetidos', () =>
    expect(() => normalize({ type: 'cnpj', value: '00000000000000' })).toThrow(
      TypeError,
    ))
...
describe('plugin: cep', () => {
  ...
  it('lança para CEP incompleto', () =>
    expect(() => normalize({ type: 'cep', value: '0131010' })).toThrow(
      TypeError,
    ))
})
...
describe('plugin: phone', () => {
  it('formata celular (11 dígitos)', () =>
    expect(normalize({ type: 'phone', value: '11987654321' })).toBe(
      '(11) 98765-4321',
    ))
  it('formata fixo (10 dígitos)', () =>
    expect(normalize({ type: 'phone', value: '1134567890' })).toBe(
      '(11) 3456-7890',
    ))
  it('lança para número inválido', () =>
    expect(() => normalize({ type: 'phone', value: '123' })).toThrow(TypeError))
})

// ─── Slug ────────────────────────────────────────────────────────────────────
describe('plugin: slug', () => {
  it('gera slug básico', () =>
    expect(normalize({ type: 'slug', value: 'Olá Mundo Legal!' })).toBe(
      'ola-mundo-legal',
    ))
  it('remove acentos', () =>
    expect(normalize({ type: 'slug', value: 'Café & Chá' })).toBe('cafe-cha'))
  it('suporta separador customizado', () =>
    expect(
      normalize({
        type: 'slug',
        value: 'Post Incrível',
...
describe('plugin: email', () => {
  it('normaliza email (lowercase + trim)', () => {
    expect(normalize({ type: 'email', value: '  User@Example.COM  ' })).toBe(
      'user@example.com',
    )
  })
  it('aceita email válido', () => {
    expect(normalize({ type: 'email', value: 'joao@empresa.com.br' })).toBe(
      'joao@empresa.com.br',
    )
  })
  it('lança para email inválido', () => {
    expect(() => normalize({ type: 'email', value: 'invalido' })).toThrow(
      TypeError,
    )
    expect(() => normalize({ type: 'email', value: 'sem@dominio' })).toThrow(
      TypeError,
    )
  })
  it('validate funciona', () => {
...
describe('plugin: color', () => {
  it('normaliza hex shorthand', () =>
    expect(normalize({ type: 'color', value: '#abc' })).toBe('#aabbcc'))
  it('normaliza hex completo', () =>
    expect(normalize({ type: 'color', value: '#FF0080' })).toBe('#ff0080'))
  it('converte rgb() para hex', () =>
    expect(normalize({ type: 'color', value: 'rgb(255,0,128)' })).toBe(
      '#ff0080',
    ))
  ...
  it('lança para formato inválido', () => {
    expect(() => normalize({ type: 'color', value: 'vermelho' })).toThrow(
      TypeError,
    )
  })
})
