...
// Cache: 1 objeto imutável por tipo, reutilizado em todos os acessos (micro-opt)
...
function buildAccessor(type) {
    ...
    const validate = (value, options) => {
        try   { return { valid: true,  value: normalize(value, options), error: null } }
        catch (err) { return { valid: false, value: null, error: err.message } }
    }
...
/**
 * Namespace de metadados — handlers.$
 *
 * @example
 ...
 *   handlers.name.parse('  joao  ')          // alias de normalize
 *   handlers.cpf.safe('111.444.777-35')      // { valid: true, ... }
 *   handlers.cpf.validate('000.000.000-00')  // { valid: false, ... }
 *
 * Introspecção:
 *   handlers.has('cpf')     // true
 *   handlers.types          // ['name', 'number', 'date', ...]
 *   handlers.$              // namespace meta
 *   handlers.$.types        // idem
 */
export const handlers = new Proxy({}, {
    get(_, prop) {
        ...
        if (prop === 'has')   return (t) => registry.has(formatType(t))
