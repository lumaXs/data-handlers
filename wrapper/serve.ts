...
type SystemResponseFactory = (req: { url: string }, message?: string) => Response
 ...
 */
export function requireAuth<TUser = unknown>(req: BunLikeRequest<TUser>): Response | null {
...
async function runLogger(entry: LogEntry, opts: LoggerOptions): Promise<void> {
    ...
    if (level === 'standard' || level === 'verbose') {
        ...
        const keep = ['content-type', 'authorization', 'accept', 'user-agent', 'x-request-id', 'x-forwarded-for', 'origin', 'referer']
        for (const k of keep) {
...
class Server<TUser = unknown> {
    ...
    #parseRoute(path: string): { regex: RegExp; keys: string[] } {
        const keys: string[] = []
        const pattern = path.replace(/\/\*/g, '(?:/.*)?').replace(/:([a-zA-Z_]+)/g, (_, key: string) => {
            keys.push(key)
            return '([^/]+)'
        })
    ...
    #checkRateLimit(ip: string): RateLimitResult {
        ...
        const reset = oldestTimestamp ? Math.ceil((oldestTimestamp + opts.windowMs) / 1000) : Math.ceil((now + opts.windowMs) / 1000)
    ...
    #applyRateLimitHeaders(nodeRes: http.ServerResponse, info: RateLimitResult): void {
    ...
    async #runMiddlewareChain(middlewares: Middleware<TUser>[], req: BunLikeRequest<TUser>): Promise<Response | null> {
    ...
    #handleWebSocketUpgrade(nodeReq: http.IncomingMessage, socket: import('net').Socket, head: Buffer, upgradeData: unknown): void {
        ...
        socket.on('data', (chunk: Buffer) => {
            ...
            while (buffer.length >= 2) {
                ...
                const maskKey = masked ? buffer.slice(headerLength, headerLength + 4) : null
    ...
    #createServer(): http.Server {
        const server = http.createServer(async (nodeReq: http.IncomingMessage, nodeRes: http.ServerResponse) => {
            ...
            const log = async (response: Response): Promise<void> => {
                ...
                if (this.#loggerOpts.ignore && isIgnored(pathname, this.#loggerOpts.ignore)) return
            ...
            try {
                ...
                for (const route of this.#routes) {
                    ...
                    if (typeof handler === 'object') {
                        ...
                        const methodHandler = (handler as MethodMap<TUser>)[method as keyof MethodMap<TUser>]
        ...
        if (this.#wsHandlers) {
            server.on('upgrade', async (nodeReq: http.IncomingMessage, socket: import('net').Socket, head: Buffer) => {
                try {
    ...
    listen(optionsOrCallback?: ListenOptions | (() => void), maybeCallback?: () => void): this {
        ...
        if (typeof optionsOrCallback === 'function') {
            ...
            logger = optionsOrCallback.logger !== undefined ? this.#resolveLogger(optionsOrCallback.logger) : logger
 ...
 * })
 */
export function serve<TUser = unknown>(options?: ServeOptions<TUser>): Server<TUser> {
