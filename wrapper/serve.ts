...
class Server<TUser = unknown> {
  ...
  #createServer(): http.Server {
      ...
      async (nodeReq: http.IncomingMessage, nodeRes: http.ServerResponse) => {
        try {
          ...
          request.upgrade = (data?: unknown): Response => {
            ...
            const res = new Response(null, { status: 101 }) as UpgradeResponse;
            (res as unknown as Record<symbol, unknown>)[UPGRADE_SIGNAL] = data;
            return res;
          };
          ...
          for (const route of this.#routes) {
            ...
            if (typeof handler === "object") {
              ...
              const methodHandler =
                (handler as MethodMap<TUser>)[method as keyof MethodMap<TUser>];
          ...
          // ── WebSocket Upgrade ──────────────────────────────────────────────
          // O pipeline de WS roda no evento 'upgrade' do servidor.
          // Se req.upgrade() foi chamado num handler HTTP normal, apenas ignora.
          ...
          try {
            const errResponse =
              ...
              systemResponses[500]!({ url: `http://${nodeReq.headers.host ?? "localhost"}${nodeReq.url}` });
    ...
    // IMPORTANTE: o Node.js emite 'upgrade' em vez de 'request' para conexões
    // WebSocket — o HTTP handler nunca roda. Por isso rodamos o pipeline
    // completo (auth, middleware, routing) aqui antes do handshake.
    if (this.#wsHandlers) {
      server.on(
        "upgrade",
        async (
          ...
          try {
            ...
            // ── Monta Request igual ao HTTP handler ────────────────────────
            const host = nodeReq.headers.host ?? "localhost";
            const url  = `http://${host}${nodeReq.url}`;
            const pathname = new URL(url).pathname;
