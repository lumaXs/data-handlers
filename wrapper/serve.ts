...
export const systemResponses: Record<number, SystemResponseFactory> = {
   ...
   503: (req, message = 'Service Unavailable') =>
      Response.json(
         {
            statusCode: 503,
...
class Server<TUser = unknown> {
   ...
   #openConnections: Set<import('net').Socket> = new Set()
   #openWebSockets: Set<WSContext<unknown>> = new Set()
         ...
         server.on(
            ...
            async (
               ...
               try {
                  ...
                  if (globalResult) {
                     socket.write(
                        `HTTP/1.1 ${globalResult.status} Error\r\n\r\n`,
                     )
                     socket.destroy()
                     return
                  }
                  ...
                  for (const route of this.#routes) {
                     ...
                     if (typeof handler === 'object') {
                        ...
                        if (routeResult) {
                           socket.write(
                              `HTTP/1.1 ${routeResult.status} Error\r\n\r\n`,
                           )
                           socket.destroy()
                           return
                        }
    ...
    */
   async gracefulShutdown(overrides?: Partial<ShutdownOptions>): Promise<void> {
      ...
      if (this.#loggerOpts) {
         console.info(
            `\x1b[33m[shutdown]\x1b[0m \x1b[90mstarting graceful shutdown (timeout: ${timeout}ms)\x1b[0m`,
         )
      }
      ...
      await new Promise<void>(resolve => {
         ...
         if (this.#loggerOpts) {
            console.info(
               `\x1b[90m[shutdown] waiting for ${this.#activeRequests} active request(s)...\x1b[0m`,
            )
         }
         ...
         const forceTimer = setTimeout(() => {
            ...
            if (this.#loggerOpts) {
               console.warn(
                  `\x1b[31m[shutdown] timeout reached — forcing close (${this.#activeRequests} request(s) still active)\x1b[0m`,
               )
            }
            resolve()
         }, timeout)
   ...
   listen(
      ...
      this.#server.listen(this.#port, this.#host, () => {
         ...
         if (banner) {
            ...
            console.info(
               `${ANSI.dim}server listening on${ANSI.reset} ${ANSI.cyan}${href}${ANSI.reset}`,
            )
         }
