...
describe('normalize()', () => {
  describe('type: "name"', () => {
    ...
    it('normaliza com espaços extras', () => {
      expect(normalize({ type: 'name', value: '  joão   da   silva  ' })).toBe(
        'João da Silva',
      )
    })
    it('normaliza caixa alta', () => {
      expect(normalize({ type: 'name', value: 'MARIA SILVA' })).toBe(
        'Maria Silva',
      )
    })
    it('respeita conectivos pt-BR', () => {
      expect(normalize({ type: 'name', value: 'maria das dores' })).toBe(
        'Maria das Dores',
      )
    })
    ...
    it('mensagem contém prefixo correto', () => {
      expect(() => normalize({ type: 'name', value: '' })).toThrow(
        '[normalize:name]',
      )
    })
  })
  ...
  describe('type: "number"', () => {
    it('formata com locale pt-BR (padrão)', () => {
      expect(normalize({ type: 'number', value: 1234567.89 })).toBe(
        '1.234.567,89',
      )
    })
    it('formata como moeda BRL', () => {
      expect(
        normalize({
          type: 'number',
          value: 9.9,
          options: { locale: 'pt-BR', style: 'currency', currency: 'BRL' },
        }),
      ).toMatch(/R\$/)
    })
    ...
    it('lança TypeError para string numérica', () => {
      expect(() => normalize({ type: 'number', value: '123' })).toThrow(
        TypeError,
      )
    })
    it('lança RangeError para NaN', () => {
      expect(() => normalize({ type: 'number', value: NaN })).toThrow(
        RangeError,
      )
    })
    it('lança RangeError para Infinity', () => {
      expect(() => normalize({ type: 'number', value: Infinity })).toThrow(
        RangeError,
      )
    })
    it('mensagem contém prefixo correto', () => {
  ...
  describe('type: "date"', () => {
    ...
    it('formata data longa pt-BR', () => {
      expect(
        normalize({
          type: 'date',
          value: new Date(2026, 2, 1),
          options: { locale: 'pt-BR', dateStyle: 'long' },
        }),
    ...
    it('lança TypeError para string inválida', () => {
      expect(() => normalize({ type: 'date', value: 'data-invalida' })).toThrow(
        TypeError,
      )
    })
    it('mensagem contém prefixo correto', () => {
      expect(() => normalize({ type: 'date', value: 'invalido' })).toThrow(
        '[normalize:date]',
      )
    })
    ...
    it('format: iso com Date object', () => {
      ...
      expect(
        normalize({ type: 'date', value: d, options: { format: 'iso' } }),
      ).toBe(d.toISOString())
    })
  ...
  describe('tipo desconhecido / case-insensitive', () => {
    it('lança TypeError para tipo não registrado', () => {
      expect(() => normalize({ type: 'inexistente', value: 'x' })).toThrow(
        TypeError,
      )
    })
    ...
    it('type ignora espaços extras', () => {
      expect(normalize({ type: '  name  ', value: 'john doe' })).toBe(
        'John Doe',
      )
    })
  })
...
describe('type: "password"', () => {
  it('aceita senha válida com padrões default', () => {
    expect(normalize({ type: 'password', value: 'Senha@123' })).toBe(
      'Senha@123',
    )
  })
  it('lança se menor que minLength (default 8)', () => {
    expect(() => normalize({ type: 'password', value: 'Ab@1' })).toThrow(
      '[normalize:password]',
    )
  })
  it('lança se não tiver maiúscula', () => {
    expect(() => normalize({ type: 'password', value: 'senha@123' })).toThrow(
      '[normalize:password]',
    )
  })
  ...
  it('lança se não tiver especial', () => {
    expect(() => normalize({ type: 'password', value: 'SenhaABC1' })).toThrow(
      '[normalize:password]',
    )
  })
  ...
  it('lança para valor não-string', () => {
    expect(() => normalize({ type: 'password', value: 12345678 })).toThrow(
      '[normalize:password]',
    )
  })
  it('mensagem contém prefixo correto', () => {
    expect(() => normalize({ type: 'password', value: 'fraca' })).toThrow(
      '[normalize:password]',
    )
  })
})
...
describe('type: "url"', () => {
  it('normaliza URL simples', () => {
    expect(normalize({ type: 'url', value: 'https://example.com' })).toBe(
      'https://example.com/',
    )
  })
  it('lowercase no host', () => {
    expect(normalize({ type: 'url', value: 'HTTPS://EXAMPLE.COM/path' })).toBe(
      'https://example.com/path',
    )
  })
  it('preserva path', () => {
    expect(
      normalize({ type: 'url', value: 'https://example.com/api/v1' }),
    ).toBe('https://example.com/api/v1')
  })
  it('preserva query string', () => {
    expect(
      normalize({ type: 'url', value: 'https://example.com/api?foo=bar' }),
    ).toBe('https://example.com/api?foo=bar')
  })
  it('aceita http', () => {
    expect(normalize({ type: 'url', value: 'http://example.com' })).toBe(
      'http://example.com/',
    )
  })
  it('lança para URL inválida', () => {
    expect(() => normalize({ type: 'url', value: 'nao-e-url' })).toThrow(
      '[normalize:url]',
    )
  })
  it('lança para protocolo não permitido (ftp)', () => {
    expect(() =>
      normalize({ type: 'url', value: 'ftp://example.com' }),
    ).toThrow('[normalize:url]')
  })
  it('aceita protocolo customizado via options', () => {
    expect(
      normalize({
        ...
        options: { protocols: ['ftp'] },
      }),
    ).toBe('ftp://example.com/')
  })
  it('lança para string vazia', () => {
    expect(() => normalize({ type: 'url', value: '' })).toThrow(
      '[normalize:url]',
    )
  })
  it('mensagem contém prefixo correto', () => {
    expect(() => normalize({ type: 'url', value: 'invalida' })).toThrow(
      '[normalize:url]',
    )
  })
})
...
describe('type: "uuid"', () => {
  ...
  it('retorna uppercase com options.uppercase: true', () => {
    expect(
      normalize({ type: 'uuid', value: V4, options: { uppercase: true } }),
    ).toBe(V4.toUpperCase())
  })
  it('valida versão específica v4', () => {
    expect(
      normalize({ type: 'uuid', value: V4, options: { version: 4 } }),
    ).toBe(V4)
  })
  it('valida versão específica v1', () => {
    expect(
      normalize({ type: 'uuid', value: V1, options: { version: 1 } }),
    ).toBe(V1)
  })
  it('lança quando versão não bate', () => {
    expect(() =>
      normalize({ type: 'uuid', value: V1, options: { version: 4 } }),
    ).toThrow('[normalize:uuid]')
  })
  it('lança para UUID inválido', () => {
    expect(() => normalize({ type: 'uuid', value: 'nao-e-uuid' })).toThrow(
      '[normalize:uuid]',
    )
  })
  it('lança para string vazia', () => {
    expect(() => normalize({ type: 'uuid', value: '' })).toThrow(
      '[normalize:uuid]',
    )
  })
  it('lança para versão não suportada', () => {
    expect(() =>
      normalize({ type: 'uuid', value: V4, options: { version: 9 } }),
    ).toThrow('[normalize:uuid]')
  })
  it('mensagem contém prefixo correto', () => {
    expect(() => normalize({ type: 'uuid', value: 'invalido' })).toThrow(
      '[normalize:uuid]',
    )
  })
})
...
describe('type: "any"', () => {
  ...
  it('lança para null', () => {
    expect(() => normalize({ type: 'any', value: null })).toThrow(
      '[normalize:any]',
    )
  })
  it('lança para undefined', () => {
    expect(() => normalize({ type: 'any', value: undefined })).toThrow(
      '[normalize:any]',
    )
  })
  ...
  it('funciona no schema com demandType', () => {
    const s = schema({
      age: { type: 'any', options: { demandType: 'number' } },
    })
  ...
  it('transform recebe string e retorna uppercase', () => {
    expect(
      normalize({
        type: 'any',
        value: 'oi',
        options: { transform: v => v.toUpperCase() },
      }),
    ).toBe('OI')
  })
  it('transform roda após demandType', () => {
    expect(
      normalize({
        type: 'any',
        value: 5,
        options: { demandType: 'number', transform: v => v * 3 },
      }),
    ).toBe(15)
  })
  it('lança se demandType falha antes do transform', () => {
    expect(() =>
      normalize({
        type: 'any',
        value: 'oi',
        options: { demandType: 'number', transform: v => v },
      }),
    ).toThrow('[normalize:any]')
  })
  it('lança se transform não for função', () => {
    expect(() =>
      normalize({ type: 'any', value: 42, options: { transform: 'nope' } }),
    ).toThrow('[normalize:any]')
  })
  ...
  it('funciona no schema com transform', () => {
    const s = schema({
      score: {
        type: 'any',
        options: { demandType: 'number', transform: v => Math.min(100, v) },
      },
    })
